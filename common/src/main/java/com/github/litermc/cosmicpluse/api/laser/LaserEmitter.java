...
import com.github.litermc.cosmicpluse.util.SerializeUtil;
...
public abstract class LaserEmitter {
	...
	public static LaserEmitter parseFromNBT(Level level, CompoundTag data) {
		final byte type = data.getByte("Type");
		final LaserEmitter emitter = switch (type) {
			case BlockLaserEmitter.TYPE_ID -> new BlockLaserEmitter(level);
			case EntityLaserEmitter.TYPE_ID -> new EntityLaserEmitter(level);
